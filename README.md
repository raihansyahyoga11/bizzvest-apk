...
Modul ini merupakan implementasi halaman _homepage_ dari aplikasi BizzVest. Beberapa bagian utama pada modul ini yaitu info mengenai BizzVest, dan fitur kirim pesan ke BizzVest. Secara umum, modul ini akan mengimplementasikan _widget layout_ seperti Center, Padding, dan lain sebagainya. Info mengenai BizzVest akan mengimplementasikan beberapa kombinasi widget, contohnya widget Text. Fitur kirim pesan ke BizzVest akan mengimplementasikan _widget_ Form untuk menerima input pesan dari pengguna, lalu ditambah tombol “Kirim Pesan” yang menerapkan Event Handling yaitu onPressed, dan akan dilakukan pemanggilan _Asynchronus_ ke Web Service Django untuk memberitahukan bahwa pesan telah terkirim.

### 2) Modul my_profile (Raihansyah Yoga Adhitama)
Modul my_profil merupakan bentuk implementasi profil user dari yang tadinya diperuntukkan bagi website menjadi mobile app dengan bantuan framework flutter. Modul my_profil ini menyimpan data-data mengenai profil user seperti nama lengkap, username, foto profil, alamat, jenis kelamin, deskripsi, serta status user sebagai pengusaha maupun investor. Pengguna dapat mengubah data-data pada profil tersebut dengan menekan tombol ‘ubah profil’ dan menyimpan data hasil ubahan tersebut ke database django.

Modul ini mengimplementasikan widget-widget umum seperti container, Text, Column, Row, Icon, Button, TextFormField, dan sebagainya. Widget utama yang frekuensi penggunaannya cukup sering adalah TextFromField yang akan menerima data-data yang dimasukkan oleh perngguna dan juga button yang didalamnya terdapat Event Handling yang merespon OnPressed ketika user menekan “Edit profil”, “simpan perubahan”, maupun “Batalkan perubahan”. Tidak lupa juga, modul ini menerapkan pemanggilan Asynchronous ke Web Service Django. Implementasi yang pertama dari pemanggilan tersebut adalah untuk memanggil status dari user dimana terdapat status pengusaha dan investor. Selanjutnya, implementasi kedua adalah ketika user menerima informasi berupa email serta foto profil yang dipakai oleh pengguna. Pemanggilan tersebut akan menyebabkan Web Service Django mengembalikan data response dalam format JSON yang nantinya akan ditampilkan pada laman profil pengguna di mobile app.
